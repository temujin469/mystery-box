...
export function BoxCard({ box }: Props) {
  ...
  const isNew = (() => {
    if (!box.created_at) return false;
  ...
  return (
    <Card
      key={box.name}
      className="box-card relative border-0 py-0 rounded-md overflow-hidden bg-card/20 group"
    >
      {/* Price Label */}
      {isNew && (
        <div
      ...
      <div className="relative">
        <div
          className={clsx(
            "duration-200 group-hover:w-[75%] group-hover:h-[75%] blur-2xl rounded-full w-[60%] h-[60%] z-0 absolute top-[12%] right-[50%] translate-x-[50%]",
            rarityColors.bg
          )}
        ></div>
        {/* Box image & title */}
      ...
      <div
        className={clsx(
          `font-bold text-[12px] md:text-sm z-10 h-[50px] flex items-center pl-4 md:pl-6 bg-gradient-to-r bg-clip-text text-transparent ${rarityColors.gradient}`
        )}
      >
